...
flags.DEFINE_integer("max_predictions_per_seq", 32,
...
def main(_):
    ...
    for input_pattern in FLAGS.input_file.split(","):
        # tf.compat.v1.gfile.Glob: 查找匹配pattern的文件并以列表的形式返回，
...
if __name__ == "__main__":
    # 注意这里的for循环不太起作用，所以需要一个一个来运行
    for i in range(10):
        flags.DEFINE_string("input_file", "train_split_data\\train_split_data_" + str(i) + ".csv",
                            "Input raw text file (or comma-separated list of files).")

        flags.DEFINE_string(
            "output_file", "records\\train_"+str(i)+".tfrecord",
            "Output TF example file (or comma-separated list of files).")

        flags.DEFINE_string("vocab_file", "bert-mini\\vocab.txt",
